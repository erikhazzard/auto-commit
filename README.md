...
Large sweeps are compacted before model invocation without changing the frozen Git tree. A fully removed directory containing at least 20 files becomes one digest-backed deletion entry; if the remaining evidence would still overfill the model budget, coherent path cohorts are progressively replaced by bounded status/sample/type/digest summaries. Every raw path still participates in safety checks and work-spec discovery. Recognized dependency lockfiles and individual blobs above the detailed-evidence budget remain staged and are committed exactly, but their bodies are omitted from model context and represented as metadata-only supporting changes. Every omitted blob retained in the frozen tree is still streamed through the secret scanner with bounded memory before a model runs. Deleted base blobs whose bodies are omitted reach neither the model nor the commit. There is no fixed changed-file count, model-packet-size, or individual-blob-size rejection: evidence becomes more summarized as the sweep grows.
